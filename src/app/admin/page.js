...
export default function AdminPage() {
  ...
  const handleLogin = (e) => {
    e.preventDefault()
    if (password === ADMIN_PASSWORD) { setIsAuthenticated(true); localStorage.setItem('tr-admin-auth', 'true') }
    else alert('Wrong password!')
  }

  const handleLogout = () => { setIsAuthenticated(false); localStorage.removeItem('tr-admin-auth') }

  const fetchAllData = async () => {
    setLoading(true)
    const [b, bk, mx, msg, av, md, rm, si] = await Promise.all([
    ...
    setRecentMixes(rm.data || []); setSiteImages(si.data || [])
    setLoading(false)
  }
      ...
      <main className="min-h-screen bg-[#050505] text-white flex items-center justify-center px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative bg-white/[0.02] border border-white/10 rounded-2xl md:rounded-3xl p-6 md:p-8 max-w-md w-full">
          <div className="text-center mb-6 md:mb-8">
            <span className="text-3xl md:text-4xl block mb-3 md:mb-4">🔐</span>
            <h1 className="text-xl md:text-2xl font-bold mb-2">Admin Panel</h1>
            <p className="text-gray-500 text-xs md:text-sm">Enter password to continue</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:border-[#8B5CF6]/50 transition" />
            <button type="submit" className="w-full bg-[#8B5CF6] hover:bg-[#7C3AED] py-3 rounded-xl font-semibold transition text-sm md:text-base">Login</button>
          </form>
      ...
      <header className="bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/10 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <span className="hidden md:inline text-xs text-gray-500 bg-white/5 px-2 py-1 rounded-full">Dashboard</span>
          </div>
          <div className="flex items-center gap-2 md:gap-4">
            <button onClick={fetchAllData} className="text-gray-400 hover:text-white transition text-xs md:text-sm flex items-center gap-1 md:gap-2">🔄 <span className="hidden sm:inline">Refresh</span></button>
