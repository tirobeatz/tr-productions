...
import { Parallax } from '@/app/components/animations'
...
export default function MixingPage() {
  ...
  const MIX_MASTER_PRICE = 150
  const RUSH_FEE = 30

  const stats = [
    { value: '100+', label: t('mixing.stats.tracksMixed'), icon: '🎚️' },
    { value: '120+', label: t('mixing.stats.happyArtists'), icon: '🎤' },
    { value: '48h', label: t('mixing.stats.avgDelivery'), icon: '⚡' },
    { value: '100%', label: t('mixing.stats.satisfaction'), icon: '⭐' }
  ]

  // Genre names are kept as English terms; only descriptions are translated.
  const genres = [
    { name: 'Hip-Hop', icon: '🎤', desc: t('mixing.genres.hiphop'), color: '#8B5CF6' },
    { name: 'Trap', icon: '🔥', desc: t('mixing.genres.trap'), color: '#EF4444' },
    { name: 'Drill', icon: '💀', desc: t('mixing.genres.drill'), color: '#6B7280' },
    { name: 'R&B', icon: '💜', desc: t('mixing.genres.rnb'), color: '#EC4899' },
    { name: 'Pop', icon: '✨', desc: t('mixing.genres.pop'), color: '#F59E0B' },
    { name: 'Afrobeat', icon: '🌍', desc: t('mixing.genres.afrobeat'), color: '#10B981' }
  ]

  const process = [
    { step: '01', title: t('mixing.process.uploadTitle'), desc: t('mixing.process.uploadDesc'), icon: '📤', time: t('mixing.process.day1') },
    { step: '02', title: t('mixing.process.reviewTitle'), desc: t('mixing.process.reviewDesc'), icon: '🎧', time: t('mixing.process.day1') },
    { step: '03', title: t('mixing.process.mixTitle'), desc: t('mixing.process.mixDesc'), icon: '🎚️', time: t('mixing.process.day2') },
    { step: '04', title: t('mixing.process.masterTitle'), desc: t('mixing.process.masterDesc'), icon: '💎', time: t('mixing.process.day23') },
    { step: '05', title: t('mixing.process.deliverTitle'), desc: t('mixing.process.deliverDesc'), icon: '✅', time: t('mixing.process.day3') }
  ]

  const whyMe = [
    { icon: '👤', title: t('mixing.why.personalTitle'), desc: t('mixing.why.personalDesc'), highlight: t('mixing.why.personalHighlight') },
    { icon: '💬', title: t('mixing.why.directTitle'), desc: t('mixing.why.directDesc'), highlight: t('mixing.why.directHighlight') },
    { icon: '🎵', title: t('mixing.why.genreTitle'), desc: t('mixing.why.genreDesc'), highlight: t('mixing.why.genreHighlight') },
    { icon: '💰', title: t('mixing.why.fairTitle'), desc: t('mixing.why.fairDesc'), highlight: t('mixing.why.fairHighlight') }
  ]

  const getInitials = (name) => name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || '?'

  const faqs = [
    { q: t('mixing.faq.q1'), a: t('mixing.faq.a1') },
    { q: t('mixing.faq.q2'), a: t('mixing.faq.a2') },
    { q: t('mixing.faq.q3'), a: t('mixing.faq.a3') },
    { q: t('mixing.faq.q4'), a: t('mixing.faq.a4') },
    { q: t('mixing.faq.q5'), a: t('mixing.faq.a5') },
    { q: t('mixing.faq.q6'), a: t('mixing.faq.a6') }
  ]

  const trustBadges = [
    { icon: '⚡', title: t('mixing.trust.fastTitle'), sub: t('mixing.trust.fastSub') },
    { icon: '🔄', title: t('mixing.trust.revisionTitle'), sub: t('mixing.trust.revisionSub') },
    { icon: '💬', title: t('mixing.trust.contactTitle'), sub: t('mixing.trust.contactSub') },
    { icon: '✅', title: t('mixing.trust.satisfactionTitle'), sub: t('mixing.trust.satisfactionSub') }
  ]
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok || data.error) throw new Error(data.error || t('mixing.form.errSubmit'))

      setSubmitted(true)
    } catch (err) { setSubmitError(err.message || t('mixing.form.errSubmit')) }
        ...
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            >
              ...
              >
                {t('mixing.hero.titlePre')}{' '}
                <span className="relative inline-block">
                  <span className="text-[#8B5CF6]">{t('mixing.hero.titleHighlight')}</span>
                  <motion.span
              ...
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-8 md:mb-10">
                <motion.a
                  ...
                  whileTap={{ scale: 0.98 }}
                >
                  <span className="relative z-10">{t('mixing.hero.ctaStart')} - €{MIX_MASTER_PRICE}</span>
                  <motion.div
            ...
            >
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4 md:mb-6">
                  <div>
                    <p className="text-xs md:text-sm text-gray-500">{t('mixing.player.before')}</p>
                    <p className="font-semibold text-sm md:text-base">{t('mixing.player.hear')}</p>
                  </div>
                  <AnimatePresence>
                    {isPlaying && (
                      <motion.div
                ...
                <div className="flex bg-white/5 rounded-full p-1 mb-4 md:mb-6 relative">
                  ...
                  {['raw', 'mixed'].map(mode => (
                    ...
                    >
                      {mode === 'raw' ? `🔇 ${t('mixing.player.raw')}` : `🔊 ${t('mixing.player.mixed')}`}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex justify-center mb-3 md:mb-4">
                  <motion.span
                    ...
                    layout
                  >
                    {audioMode === 'mixed' ? `✨ ${t('mixing.player.proMix')}` : `📼 ${t('mixing.player.unprocessed')}`}
                  </motion.span>
                </div>
      ...
      <section className="relative py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.why.label')}</p>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-3 md:mb-4">{t('mixing.why.title')}</h2>
            <p className="text-gray-500 max-w-lg mx-auto text-sm md:text-base">{t('mixing.why.subtitle')}</p>
          </motion.div>
      ...
      <section className="relative py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.genres.label')}</p>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-3 md:mb-4">{t('mixing.genres.title')}</h2>
            <p className="text-gray-500 max-w-lg mx-auto text-sm md:text-base">{t('mixing.genres.subtitle')}</p>
          </motion.div>
      ...
      <section id="process" className="relative py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.process.label')}</p>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-3 md:mb-4">{t('mixing.process.title')}</h2>
            <p className="text-gray-500 text-sm md:text-base">{t('mixing.process.subtitle')}</p>
          </motion.div>
        ...
        <section className="relative py-16 md:py-24 px-4 md:px-6">
          <div className="max-w-6xl mx-auto">
            <motion.div
              ...
              variants={fadeUp}
            >
              <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.recent.label')}</p>
              <h2 className="text-4xl md:text-6xl font-bold tracking-tight">{t('mixing.recent.title')}</h2>
            </motion.div>
                    ...
                    <AnimatePresence>
                      {playingMixId === m.id && (
                        ...
                        >
                          ...
                          <span className="text-xs">{t('mixing.recent.playing')}</span>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
        ...
        <section className="relative py-16 md:py-24 px-4 md:px-6">
          <div className="max-w-6xl mx-auto">
            <motion.div
              ...
              variants={fadeUp}
            >
              <p className="text-[#1DB954] font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.releases.label')}</p>
              <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-3 md:mb-4">{t('mixing.releases.title')}</h2>
              <p className="text-gray-500 max-w-lg mx-auto text-sm md:text-base">{t('mixing.releases.subtitle')}</p>
            </motion.div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <span className="text-gray-400 text-base md:text-lg">{t('mixing.pricing.perTrack')}</span>
              </motion.div>
            </motion.div>
            ...
            >
              ...
              >
                <p className="font-semibold mb-1 md:text-lg">{t('mixing.pricing.standard')}</p>
                <p className="text-gray-500 text-sm">{t('mixing.pricing.standardTime')}</p>
                <p className="text-[#8B5CF6] font-bold mt-2">{t('mixing.pricing.included')}</p>
              </motion.div>
              <motion.div
              ...
              >
                <p className="font-semibold mb-1 md:text-lg flex items-center justify-center gap-2">
                  {t('mixing.pricing.rush')} <span className="text-lg">⚡</span>
                </p>
                <p className="text-gray-500 text-sm">{t('mixing.pricing.rushTime')}</p>
                <p className="text-[#8B5CF6] font-bold mt-2">+€{RUSH_FEE}</p>
              </motion.div>
            </motion.div>

            <motion.ul className="space-y-3 mb-8" variants={fadeUp}>
              {[t('mixing.pricing.f1'), t('mixing.pricing.f2'), t('mixing.pricing.f3'), t('mixing.pricing.f4'), t('mixing.pricing.f5')].map((item, i) => (
                <motion.li
                  key={i}
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
            >
              {t('mixing.cta.getStartedNow')}
            </motion.a>
            <p className="text-center text-gray-500 text-xs mt-4">{t('mixing.pricing.discount')}</p>
          </motion.div>
        </div>
      </section>
        ...
        <section className="relative py-16 md:py-24 px-4 md:px-6">
          <div className="max-w-5xl mx-auto">
            <motion.div
              ...
              variants={fadeUp}
            >
              <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.reviews.label')}</p>
              <h2 className="text-4xl md:text-6xl font-bold tracking-tight">{t('mixing.reviews.title')}</h2>
            </motion.div>
      ...
      <section id="submit" className="relative py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-2xl mx-auto">
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.form.label')}</p>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-3 md:mb-4">{t('mixing.form.title')}</h2>
            <p className="text-gray-500 text-sm md:text-base">{t('mixing.form.subtitle')}</p>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold mb-2">{t('mixing.success.title')}</h3>
                  <p className="text-gray-500 mb-6 text-sm md:text-base">{t('mixing.success.text')}</p>
                  <div className="bg-white/5 rounded-2xl p-4 inline-block">
                    <p className="text-sm text-gray-400">{t('mixing.success.filesTo')}</p>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { l: t('mixing.form.nameLabel'), k: 'name', t: 'text', p: t('mixing.form.namePlaceholder') },
                      { l: t('mixing.form.emailLabel'), k: 'email', t: 'email', p: 'your@email.com' }
                    ].map(f => (
                      <div key={f.k}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">{t('mixing.form.trackLabel')} *</label>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-base md:text-sm focus:outline-none focus:border-[#8B5CF6]/50 transition"
                        placeholder={t('mixing.form.trackPlaceholder')}
                      />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">{t('mixing.form.genreLabel')} *</label>
                      <select
                        required
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-base md:text-sm focus:outline-none focus:border-[#8B5CF6]/50 transition"
                      >
                        <option value="">{t('mixing.form.genrePlaceholder')}</option>
                        {['Hip-Hop', 'Trap', 'Drill', 'R&B', 'Pop', 'Afrobeat', 'Other'].map(g => (
                          <option key={g} value={g}>{g === 'Other' ? t('mixing.form.genreOther') : g}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">{t('mixing.form.refLabel')}</label>
                    <input
                      type="text"
                      value={formData.reference}
                      onChange={(e) => setFormData({ ...formData, reference: e.target.value })}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-base md:text-sm focus:outline-none focus:border-[#8B5CF6]/50 transition"
                      placeholder={t('mixing.form.refPlaceholder')}
                    />
                  </div>

                  <div>
                    <label className="block text-sm text-gray-400 mb-2">{t('mixing.form.notesLabel')}</label>
                    <textarea
                      ...
                      placeholder={t('mixing.form.notesPlaceholder')}
                    />
                  </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold">{t('mixing.form.rushTitle')}</p>
                          <p className="text-gray-500 text-sm">{t('mixing.form.rushDesc')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                  ...
                  <div className="bg-white/5 rounded-xl p-4 flex items-center justify-between">
                    <div>
                      <span className="text-gray-400 block">{t('mixing.form.priceLabel')}</span>
                      <span className="text-gray-600 text-xs">{t('mixing.form.priceNote')}</span>
                    </div>
                    <motion.span
                  ...
                  >
                    {isSubmitting ? (
                      <>
                        ...
                        />
                        {t('mixing.form.submitting')}
                      </>
                    ) : (
                      t('mixing.form.submit')
                    )}
                  </motion.button>

                  <p className="text-center text-gray-600 text-xs">
                    {t('mixing.form.filesNote')}
                  </p>
                </motion.form>
              )}
            </AnimatePresence>
      ...
      <section className="relative py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-3xl mx-auto">
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="text-gray-500 font-medium mb-3 md:mb-4 tracking-[0.3em] uppercase text-xs">{t('mixing.faq.label')}</p>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight">{t('mixing.faq.title')}</h2>
          </motion.div>
