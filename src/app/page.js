...
export default function Home() {
  const [isMobile, setIsMobile] = useState(true) // Default true for fast mobile
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white relative overflow-x-hidden">
      ...
      <section ref={ctaRef} className="relative py-20 md:py-32 px-6">
        {/* CTA background glow - desktop only */}
        <div 
          className={`hidden md:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-[#8B5CF6] rounded-full pointer-events-none transition-all duration-1000 ${ctaVisible ? 'opacity-[0.12]' : 'opacity-0'}`}
          style={{ filter: 'blur(120px)' }}
        />
